...
!      lwork=knsq
      lwork=n*n
      if (verbose .ge. 5) write(*,*) 'in eigen, calling dsytrd'
      call dsytrd(uplo,n,matrix,kn,eigval,e,tau,work,lwork,info)
      if (verbose .ge. 5) write(*,*) 'return from dsytrd'
...
!---------------------------------------------------------------------------
!
subroutine eigen_nag(matrix,kn,n,eigvalr,eigvali)
!
! Does not assume up-down symmetry; uses Nag routine to find eigenvalues
! and eigenvectors
!
      implicit none
      integer kn,n,lwork,ifail,i
      real matrix(kn,kn)
      real work_matrix(64*n)
...
!
      ...
      lwork=64*n
      ifail=0
! remove on alpha due to lack of NAG libraries
!      call f02ebf(job,n,matrix,kn,eigvalr,eigvali,eigvecr,kn,eigveci,kn,  &
!                  work_matrix,lwork,ifail)
      return
end subroutine eigen_nag
...
subroutine eigen_comp(matrix,kn,n,eigvalr,eigvali, &
     eigvectr,eigvecti)
  implicit none
  integer kn,n,i,j
  complex matrix(n,n),hess(n,n),dum(n,n),x(n,n)
  integer iter(n)
  real eigvalr(kn),eigvali(kn),eigvectr(n,n),eigvecti(n,n)
  complex eiga(n),eigb(n)
  ...
  enddo
  hess=matrix
  call lzhes(n,hess,n,dum,n,x,n,.true.)
  call lzit(n,hess,n,dum,n,x,n,.true.,iter,eiga,eigb)

  do i=1,n
     eigvalr(i)=real(eiga(i)/eigb(i))
     eigvali(i)=imag(eiga(i)/eigb(i))
     do j=1,n

! eigvectr is really double sized matrix
        eigvectr(i,j)=real(x(i,j))
        eigvecti(i,j)=imag(x(i,j))
     enddo
  enddo

end subroutine eigen_comp

subroutine eigen_hcomp(matrix,kn,n,eigvalr,eigvali, &
     eigvectr,eigvecti,info)
! solver for eigenvalues and eigv for Hermitian matrix
  implicit none
  integer kn,n,i,j,lwork,info
  complex matrix(n,n),hess(n,n),work(10*n)
  real rwork(3*n-2)
  integer iter(n)
  real eigvalr(kn),eigvali(kn),eigvectr(n,n),eigvecti(n,n)
  ...
  lwork=10*n ! must match dimension of work
  hess=matrix
  if (kn.ne.n) then
     write(*,*) 'need kn=n on call to eigen_hcomp'
     stop
  endif

  call zheev('V','U',n,hess,n,eigvalr,work,lwork,rwork,info)
  job='v'
  uplo='up'
  lda=n
  lwork=10*n  !must match dimensions of work
  ifail=0
!  call f02haf(job,uplo,n,hess,lda,eigvalr,rwork,work,lwork,ifail)

  eigvali=0. ! hermitian eigvalues are real
  eigvectr=real(hess)
  eigvecti=aimag(hess)

end subroutine eigen_hcomp

subroutine eigen_comp2(matrix,kn,n,eigvalr,eigvali, &
     eigvectr,eigvecti,info)
! solver for eigenvalues and eigv for Hermitian matrix
  implicit none
  integer kn,n,i,j,lwork,info
  ...
  call zgeev('N','V',n,hess,n,eigv,vl,n,vr,n,work,lwork, &
       rwork,info)
