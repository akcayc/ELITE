...
module elite_data
  ...
  logical rotation          ! supplied rot, rotp profiles, use rotation in 
        !   stability calc if true
  ...
  integer mindex,mind_new        !indexing parameter =1 for updownsym
  ! =2 for .not.updownsym
  ...
  complex, dimension(:,:,:,:), allocatable :: auarr
  ! 1/02 auspline to hold spline coefficients for auarr
  complex, dimension(:,:,:,:), allocatable :: auspline
  ...
  complex, dimension(:,:,:,:), allocatable :: iuarr,iuspline
  complex, dimension(:,:,:,:), allocatable :: vuarr,ruarr
  complex, dimension(:,:), allocatable :: ddsurf1u,vsurf1u
  ...
  !                                !ballon eq. matrix--coeff of um''
  complex, dimension(:,:), allocatable :: asurfu,asurfup, &
       isurfu,isurfup
  ...
  real rav      ! Average major radius of outer flux surface
  real cssq1,cssq2     ! Square of sound speed on LCFS and inner FS
  real rva1,rva2     ! R * Alfven speed on LCFS and inner FS
  complex tmat_det ! determinant of the complex t matrix used for iteration
  ...
  real gamre ! real part of eigenvalue for rotation case
  ! this is not an input parameter, rather it's value is set using
  ! gamsq (gamre=sqrt(gamsq) or gamre=-sqrt(-gamsq) for negative gamsq)
  ...
  complex, dimension(:,:), allocatable :: ffun
  complex, dimension(:), allocatable :: aval
  real, dimension(:), allocatable :: xx
  complex, dimension(:,:), allocatable :: pmat,qmat,smat
  complex, dimension(:,:,:), allocatable :: aa,ainv
  complex, dimension(:,:), allocatable :: bb
  complex, dimension(:,:,:), allocatable :: fmat
  ...
  real, dimension(:), allocatable :: psiv_eq,pprime_eq,f_eq, &
       ...
       press_eq,rot_eq,rotp_eq,rotnorm,rotpnorm
  ! ppp_eq is the equilibrium p'', ffpp_eq=(ff')'
  ! qp_eq is the equilibrium q', qpp_eq=q'' 
  ! press_eq is the equilibrium pressure = 4 pi p
  ! rot_eq is the equilibrium rotation in rad/s
  ! rotp_eq is d(rot_eq)/dpsi
  ! rotnorm is (rot_eq-rot_eq(1))*sqrt(4*pi*rho_edge)   
...
!      which enter smat,pmat,qmat
  ...
  real rot_pedmid  ! location of rotation tanh middle in psinorm units
    ! option only for rot_model=-1, gives symmetry point in middle of tanh
  real rot_pedwid  ! width of tanh rotation profile in psinorm units
    ! used only for rot_model=-1
  real rot_sep  ! rotation in kHz at the last surface 
    ! used only for rot_model=-1
  real rot_axis ! rotation in kHz on magnetic axis
    ! used only for rot_model=-1
  real rot_expin ! inner exponent for core rotation profile
    ! used only for rot_model=-1
  real rot_expout ! outer exponent for core rotation profile
    ! used only for rot_model=-1
...
contains   
  ...
  subroutine init
    ...
    gamim=0.                  ! imaginary part of gamma for rotation
    ...
    ! defaults for rotation derived variables
    rot_const=0.
    rot_model=1
    rot_pedmid=0.975
    rot_pedwid=0.025
    ...
    outfile=39  
    if (verbose .ge. 1) open(unit=outfile,file=runname(1:lrunname)//'.out',iostat=ios)
...
! 6/20 more agressive, slow estimate based on assessment of nmvac convergence in 11 cases with a wide range of q_in and aspect ratio
          ...
          if (verbose .ge. 2) write(outfile,*) ' Entered unsupported nmvac value=',nmvac
          stop
       endif
       if (verbose .ge. 2) then
...
!       mindex=2               ! length of vectors
   ! 3.28.03 rewriting for complex variables, mindex always 1
       nmodes=nm
       mindex=1 
       mind_new=2
    endif
...
!  create normalized rotation quantities, check for consistency
    if (rotation) then
       if ((.not.dens) .or. (.not.compression) .or.(updownsym)) then
          write(*,*) 'operation with rotation currently ', &
               'requires dens=.true. and compression=.true.', &
               'and updownsym=.false.'
          stop
       endif
       rot_norm=sqrt(4.*pi*ion_mass*1.6726e-24*ne_eq(1))
       if (verbose .ge. 1) write(*,*) 'ion_mass=',ion_mass,'rot_norm=',rot_norm
       if (verbose .ge. 1) then
          open(unit=85,file=runname(1:lrunname)//'.rot',iostat=ios)
          write(85,*) 'psi_norm,rotnorm,rotpnorm,rot_eq'
       endif
       do i=1,nxinterp
!          rotnorm(i)=rot_norm*(rot_eq(i)-rot_eq(1))
!  2/06 with addition of rot*rotprime terms, no longer subtract edge value
!    (need to check where this assumption is used)
          rotnorm(i)=rot_norm*rot_eq(i)
          rotpnorm(i)=rot_norm*rotp_eq(i)

          if (verbose .ge. 1) write(85,*) psiv_eq(i)/psiv_eq(1),rotnorm(i), &
               rotpnorm(i),rot_eq(i)
       enddo
       if (rotnorm(1).ne.0.) then
          write(*,*) 'WARNING: Edge rotation not zero!! not checked!!'
          if (verbose .ge. 2) write(outfile,*) &
               'WARNING: Edge rotation not zero!! not checked!!'
       endif
!       close(85) 
!       write(*,*) 'rotnorm=',rotnorm

! initialize imaginary part of gamma
       if (gamim==0.) then
!       gamim=-nn*rotnorm(0.05*nxinterp)
          gamim=-nn*rotnorm(13)
!          gamim=nn*rotnorm(13)
!       gamim=0.
!       gamim=1.
          if (verbose .ge. 2) then
             if (verbose .ge. 1) write(*,*) 'initializing gamim=',gamim
             if (verbose .ge. 2) write(outfile,*) 'inititializing gamim=',gamim
          endif
       endif

! initialize gamre for rotation case
       if (gamsq .ge. 0.) then
          gamre=sqrt(gamsq)
       else
          gamre=-sqrt(-gamsq)
          gamsq=gamre**2  ! reset gamsq for consistency with gamre
       endif
       if (verbose .ge. 2) then
          if (verbose .ge. 1) write(*,*) 'gamre initialized to ',gamre
          if (verbose .ge. 2) write(outfile,*) 'gamre initialized to ',gamre
       endif

    endif

    ! check to make sure not running this version with updown symmetric
    !   cases except for testing purposes
    if (updownsym) then
       write(*,*) '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
       write(*,*) '***this version is intended for efficient treatment of'
       write(*,*) ' updown asymmetric equilibria.  For updown symmetric'
       write(*,*) ' cases please use version labelled elitesym.'
       write(*,*) 'Using the script elite will choose correct version'
       write(*,*) '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
  ...
  subroutine alloc_arrays
    ...
    allocate( psiv_eq(nxinterp),pprime_eq(nxinterp),f_eq(nxinterp), &
         ...
         ppp_eq(nxinterp),ffpp_eq(nxinterp),qp_eq(nxinterp), &
         qpp_eq(nxinterp),press_eq(nxinterp) )
    psiv_eq=0.; pprime_eq=0.; f_eq=0.; ffprime_eq=0.; press_eq=0.
    ...
    if (rotation) then
       allocate(rot_eq(nxinterp),rotp_eq(nxinterp),rotnorm(nxinterp), &
            rotpnorm(nxinterp) )
       rot_eq=0.; rotp_eq=0.; rotnorm=0.; rotpnorm=0.
    endif
  ...
  subroutine alloc_nustuff
    ...
    if (compression) then
       allocate( vuarr(13,lwindow,lwindow,nxinterp), &
            ruarr(4,lwindow,lwindow,nxinterp) )
       vuarr=0.; ruarr=0.
       allocate( ddsurf1u(lwindow,lwindow), vsurf1u(lwindow,lwindow) )
       ddsurf1u=0.; vsurf1u=0.
    endif
...
!    write(*,*) 'rgrid read from .xtopsi',rgrid
  ...
  subroutine rdeqdat
    ...
    real psiloc
    real aone,azero,xphalf,widthp
    real rotedge,rotped,rotcore,rexpin,rexpout
    real pconst,a_r,xped,rcoretanh,xpsi,rval,rvalp,xtoped
    real test_cssq1,test_cssq2
    real rotpp_eq
...
!            ' does not match nxinterp',nxinterp_loc,nxinterp
    ...
    if (dens) then
       ...
       if (bug(8) .ne. 0.) then
          write(*,*) '!!!Forcing flat density of bug(8)=',bug(8)
          if (verbose .ge. 2) write(outfile,*) &
               '!!!Forcing flat density of bug(8)=',bug(8)
          ne_eq=bug(8)
          neprime_eq=0.
       endif
    end if
    read(neqdat,*) dumstring
    read(neqdat,*) (press_eq(i),i=1,nxinterp)
    ! also multiply by 4pi
    press_eq=4.*pi*press_eq

!!!!!!TEMPORARY 8.22.02
!!! for now just make something up for the rotation,
!!!  later work on reading it in
    if (rotation) then
! now an input variable       rot_const=0.7e6

       test_cssq1=(te_eq(1)*1.6022d-12/(ion_mass*1.6726d-24))
       test_cssq2=(te_eq(nxinterp)*1.6022d-12/(ion_mass*1.6726d-24))

       cssq1=(press_eq(1)/(4.*pi))/(ne_eq(1)*ion_mass*1.6726d-24)
       cssq2=(press_eq(nxinterp)/(4.*pi))/(ne_eq(nxinterp)* &
            ion_mass*1.6726d-24)

       if (verbose .ge. 2) write(*,*) 'test_cs1=',sqrt(test_cssq1),'test_cs2', &
            sqrt(test_cssq2)
       
       if (verbose .ge. 2) write(6,*)' Cs LCFS,inner=',sqrt(cssq1),sqrt(cssq2), &
            ' ion_mass=',ion_mass
  ! R*Alfven frequency (B=toroidal field)
       rva1=f_eq(1)/sqrt(4.*pi*ne_eq(1)*ion_mass*1.6726d-24)
       rva2=f_eq(nxinterp)/                                     &
            sqrt(4.*pi*ne_eq(nxinterp)*ion_mass*1.6726d-24)
       if (verbose .ge. 2) write(6,*)' f1=',f_eq(1),' f2=',f_eq(nxinterp)
       if (verbose .ge. 2) write(6,*) &
            ' ne=',ne_eq(1), ne_eq(nxinterp),' ion mass=',ion_mass

       if (rot_const .eq. 0.) then
          write(*,*) 'Warning: rotation on with rot_const=0.  Will yield no rotation.'
          if (verbose .ge. 2) write(outfile,*) &
               'Warning: rotation on with rot_const=0.', &
               'Will yield no rotation.'
       endif

       if (rot_model .eq. 0) then
          write (*,*) 'rotation model rot_model must be specified for rotation=.true.'
          stop
       else if (rot_model .eq. -1) then  ! use tanh rotation profile
          if (verbose .ge. 1) write(*,*) &
               'using tanh rotation model with rot_const=',rot_const
          if (verbose .ge. 2) write(outfile,*) &
               'using tanh rotation model with rot_const=',rot_const
          aone=0.99999999
          azero=1.e-6

! if rot_axis is not set (zero), set it to 1.25 rot_const
          if ((rot_const.ne.0.) .and. (rot_axis.eq.0.)) then
             rot_axis=1.25*rot_const
             if (verbose .ge. 2) then
                write(*,*) '!!axis rotation set to 1.25*rot_const=', &
                     rot_axis,' kHz'
                if (verbose .ge. 2) write(outfile,*) &
                     '!!axis rotation set to 1.25*rot_const=', &
                     rot_axis,' kHz'
             endif
          endif
...
!          rotedge=rot_const*2.*pi*1000.
!          rotped=rot_const*16.*pi*1000.
!          rotcore=rot_const*30.*pi*1000.
!          rotedge=0.
          rotedge=rot_sep*2*pi*1000.
          rotped=rot_const*2*pi*1000.
          rotcore=rot_axis*2*pi*1000.
          rexpin=rot_expin
          rexpout=rot_expout

          if (verbose .ge. 2) then
             write(*,*) '!!!!Using tanh rotation profile!!!!' 
             write(*,*) ' tanh center=', xphalf,' tanh half width=',widthp
             write(*,*) ' rotedge=',rotedge,' rotped=',rotped,'rad/s'
             write(*,*) ' rotcore=',rotcore,' rexpin=',rexpin
             write(*,*) ' rexpout=',rexpout
             write(outfile,*) '!!!!Using tanh rotation profile!!!!' 
             write(outfile,*) ' tanh center=', xphalf,' tanh half width=',widthp
             write(outfile,*) ' rotedge=',rotedge,' rotped=',rotped,'rad/s'
             write(outfile,*) ' rotcore=',rotcore,' rexpin=',rexpin
             write(outfile,*) ' rexpout=',rexpout
          endif

          pconst=1.-tanh((aone-xphalf)/widthp)
          a_r=2.*(rotped-rotedge)/(1.+tanh(1.)-pconst)
          xped=xphalf-widthp
          rcoretanh=0.5*a_r*(1.-tanh(-xphalf/widthp)-pconst)+ &
               rotedge
!       rot_const=0.
          do i=1,nxinterp
             psiloc=psiv_eq(i)/psiv_eq(1)
             xpsi=min(psiloc,aone)
             xpsi=max(psiloc,azero)
             rval=0.5*a_r*(1.-tanh((xpsi-xphalf)/widthp)- &
                  pconst)+rotedge
             rvalp=-0.5*a_r/widthp*( &
                  1./cosh((xpsi-xphalf)/widthp)**2)
             if (rotcore.gt.0. .and. xpsi.lt.xped) then
! xtoped is proportional to psi, but normed to equal 1 at top of ped
                xtoped=xpsi/xped
                rval=rval+(rotcore-rcoretanh)* &
                     (1.-xtoped**rexpin)**rexpout
                rvalp=rvalp-(rotcore-rcoretanh)*rexpin*rexpout* &
                     xtoped**(rexpin-1.)* &
                     (1.-xtoped**rexpin)**(rexpout-1.)
             endif
             rot_eq(i)=rval
             rotp_eq(i)=rvalp/psiv_eq(1)
          enddo
          if (verbose .ge. 3) write(outfile,*) 'surface  psinorm     rot(kHz)   rotprime'
          do i=1,nxinterp
             if (verbose .ge. 3) write(outfile,1200) i,psiv_eq(i)/psiv_eq(1), &
                  rot_eq(i)/(2.*pi*1000.),rotp_eq(i)/(2.*pi*1000.)
          enddo
1200      format(i5,f12.6,2e15.6)
       else if ((rot_model .ge. 1) .and. (rot_model .le. 20)) then
          if (verbose .ge. 1) write(*,*) &
               'using polynomial rotation profile with exponent=',rot_model
          if (verbose .ge. 2) write(outfile,*) &
               'using polynomial rotation profile with exponent=', &
               rot_model
          do i=1,nxinterp
             psiloc=psiv_eq(i)/psiv_eq(1)
             rot_eq(i)=2.*pi*1000.*rot_const*(1.-psiloc**rot_model)
!             if (rot_model .eq. 1) then
!                rotp_eq(i)=-rot_const/psiv_eq(1)
!             else
                rotp_eq(i)=-rot_model*2.*pi*1000.*rot_const* &
                     psiloc**(rot_model-1)/psiv_eq(1)
                rotpp_eq=-rot_model*(rot_model-1.)*2.*pi*1000.* &
                     rot_const*psiloc**(rot_model-2)/psiv_eq(1)**2
!  temp output
!                write(*,*) psiloc,nn*qp_eq(i),rotpp_eq/rotp_eq(i), &
!                     nn*qp_eq(i)/(rotpp_eq/rotp_eq(i)),pprime_eq(i)/press_eq(i)
!             endif
          enddo
       else
          write(*,*) 'rot_model=',rot_model,' not defined'
          stop
       endif
    endif
