...
subroutine nustuff(ixinterp)
  ...
  use elite_data, only: ns,mmax,mmin,pprime_adj,ffprime_adj,f_eq, &
       ...
       omegas,omegasn,omegaspi,vuarr,g_comp,compression,te_eq,ddsurf1u, &
       vsurf1u,press_eq,ion_mass,rotation,rotpnorm,ruarr,rot_eq, &
       rav,gamscl2,kinkterms,bug,rotnorm,psigrid,funcal,verbose
  implicit none
  integer ixinterp
  ...
  real dopdomeg(ns)   ! d_omegap / d_omeg for local shear calc
  real sy_loc(ns)  ! local shear inverse length, Callen's def
  ...
  integer im,ip,np,nk
  real bsqchi(ns)
! new values for higher order rotation terms 2/06
  real rsqchi(ns)   ! d_R^2 / d_chi for kernals 70-74
  real drsqp(ns)  ! d_R^2 / d_psi for kernals 75,76
  ...
  real vlhs1real,vlhs1im,vlhs2real,vlhs2im,vrhs1real,vrhs1im
  real vrhs2real,vrhs2im,vplhs1real,vplhs1im,vplhs2real,vplhs2im
  real vprhs1real,vprhs1im,vprhs2real,vprhs2im,vprhs3real,vprhs3im
  real vprhs4real,vprhs4im,vprhs5real,vprhs5im,vprhs6real,vprhs6im
  real ddsurf1real,ddsurf1im,vsurf1real,vsurf1im
  real umvreal,umvim,umvpreal,umvpim
!  real umvgreal,umvgim
  real r1real,r1imag,rp1real,rp1imag
!  real rp2real,rp2imag,r3real,r3imag,r4real,r4imag
  real r2real,r2imag,rp2real,rp2imag
  ...
  real press_loc,rotp_loc,c_s,rot_loc


! TEMPORARY
  press_loc=press_eq(ixinterp) ! to be local pressure
!  write(*,*) 'press_loc=',press_loc

!!!!!TEMPORARY!!!!
  rotp_loc=0. ! to be local rotation/sqrt(4 pi rho_0)
  rot_loc=0.
  if (rotation) then
     rotp_loc=rotpnorm(ixinterp)
     rot_loc=rotnorm(ixinterp)
     if (verbose .ge. 3) write(outfile,*) &
          'using rotp_loc=',rotp_loc, ' rot_loc=',rot_loc
     
     c_s=9.79e5*sqrt((ti_eq(ixinterp)+te_eq(ixinterp))/ion_mass)

     if (verbose .ge. 1) write(85,*) ixinterp,'rot_eq=',rot_eq(ixinterp), &
          't_avg=',0.5*(ti_eq(ixinterp)+te_eq(ixinterp)), &
          'c_s=',c_s, &
          'R Omega/c_s',rl(1)*rot_eq(ixinterp)/c_s
  ...
  if (dens) then
!     press_loc=4.*pi*ne_eq(ixinterp)*(te_eq(ixinterp)+ti_eq(ixinterp)) &
!          *1.6022e-12
 ...
 !         'press_loc=',press_loc,'ion_mass=',ion_mass
...
!         write to screen only if q changes significantly
  ...
  if (ixinterp.eq.1) then
     if (ibench.eq.1) then
        open(76,file=runname(1:lrunname)//'bench.dat')
...
!!! TEMP
!     write(76,91) nu(i),nup(i),rl(i),zl(i)
...
91 format(5e14.6)
  ...
  if (ixinterp.eq.1 .and. bug(4).eq.1.) then
     write(75,*) nxinterp,ns
  endif
  ...
  if (bug(4) .eq. 1.) then
     write(75,*) ixinterp,psigrid(ixinterp)
     write(75,*) sqrt(bsqval)
  endif
...
!  rsqchi for higher order rotation terms 70-74
     rsqchi(i)=(rsqval(ip)-rsqval(im))/(2.*dchi)

!  d_omegp/d_omeg for local shear calculation 2/06
     dopdomeg(i)=(omegp(ip)-omegp(im))/(omeg(ip)-omeg(im))
     if (i.eq.ns) &
        dopdomeg(i)=(omegp(ip)-omegp(im))/(2.*pi+omeg(ip)-omeg(im))
     if (i.eq.1) &
        dopdomeg(i)=(omegp(ip)-omegp(im))/(omeg(ip)-omeg(im)+2.*pi) 
!  Callen's definition of local shear
     sy_loc(i)=-f_loc*(bpl(i)**2)/(qsurf*bsqval(i))*  &
          (qsurfp+qsurf*dopdomeg(i))
!!! TEMP, write local shear related values
   if (ibench .eq. 1) then
     write(76,91) nu(i),nup(i),sy_loc(i),rl(i),zl(i)
   endif
  end do
  ...
  do i=1,ns-1
     ...
     kern(67)=f_loc/(rsqval(i)*bsqval(i))
     kern(68)=rsqval(i)/bsqval(i)
     kern(69)=rsqval(i)
! 2/06 new kern's for higher order rotation terms
     kern(70)=d_loc*rsqchi(i)/(jval(i)*bsqval(i))
     kern(71)=d_loc*rsqchi(i)/(jval(i)*bsqval(i))*omegp(i)
     kern(72)=d_loc*rsqchi(i)/(jval(i)*bsqval(i))*nup(i)/nu(i)
     kern(73)=d_loc*rsqchi(i)/(jval(i)*bsqval(i))*rsqval(i)
     kern(74)=d_loc*rsqchi(i)/(jval(i)*bsqval(i))*rsqval(i)*omegp(i)
     kern(75)=d_loc*drsqp(i)/(rsqval(i)*bsqval(i))
     kern(76)=d_loc*drsqp(i)/bsqval(i)
     do ik=1,nk
        do ip=1,np
...
!        endif
        ...
        !                ,(apprealdd*dmnq+appreald)*dmpnq+apprealnd
           ...
           auarr(1,irow  ,icol  ,ixinterp)=cmplx(arealmk,aimagmk)
           auarr(2,irow  ,icol  ,ixinterp)=cmplx(areal2mk,aimag2mk)
           auarr(3,irow  ,icol  ,ixinterp)=cmplx(arealm2k,aimagm2k)
           auarr(4,irow  ,icol  ,ixinterp)=cmplx(areal2m,aimag2m)
           auarr(5,irow  ,icol  ,ixinterp)=cmplx(areal2k,aimag2k)
           auarr(6,irow  ,icol  ,ixinterp)=cmplx(arealm,aimagm)
           auarr(7,irow  ,icol  ,ixinterp)=cmplx(arealk,aimagk)
           auarr(8,irow  ,icol  ,ixinterp)=cmplx(areal0,aimag0)
           auarr(9,irow  ,icol  ,ixinterp)= &
                -nn*(qsurfp*cmplx(aprealmk,apimagmk)+ &
                qsurfpp*cmplx(apprealmk,appimagmk))
           auarr(10,irow  ,icol  ,ixinterp)= &
                -nn*(qsurfp*cmplx(apreal2mk,apimag2mk)+ &
                qsurfpp*cmplx(appreal2mk,appimag2mk))
           auarr(11,irow  ,icol  ,ixinterp)= &
                -nn*(qsurfp*cmplx(aprealm2k,apimagm2k) &
                +qsurfpp*cmplx(apprealm2k,appimagm2k))
           auarr(12,irow  ,icol  ,ixinterp)=-nn*qsurfp* &
                cmplx(apreal2m,apimag2m)
           auarr(13,irow  ,icol  ,ixinterp)=-nn*qsurfp* &
                cmplx(apreal2k,apimag2k)
           auarr(14,irow  ,icol  ,ixinterp)=-nn*qsurfp* &
                cmplx(aprealm,apimagm)
           auarr(15,irow  ,icol  ,ixinterp)=-nn*qsurfp* &
                cmplx(aprealk,apimagk)
           auarr(16,irow  ,icol  ,ixinterp)=-nn*qsurfp* &
                cmplx(apreal0,apimag0)
           auarr(17,irow  ,icol  ,ixinterp)=(nn*qsurfp)**2* &
                cmplx(apprealmk,appimagmk)
           auarr(18,irow  ,icol  ,ixinterp)=(nn*qsurfp)**2* &
                cmplx(appreal2mk,appimag2mk)
           auarr(19,irow  ,icol  ,ixinterp)=(nn*qsurfp)**2* &
                cmplx(apprealm2k,appimagm2k) 
        ...
        if (updownsym) then
           ...
           iuarr(2,irow  ,icol  ,ixinterp)=cmplx(irealm,iimagm)
           iuarr(1,irow  ,icol  ,ixinterp)=cmplx(irealk,iimagk)
           iuarr(3,irow  ,icol  ,ixinterp)=cmplx(ireal0,iimag0)
           iuarr(5,irow  ,icol  ,ixinterp)=  &
                -nn*(qsurfp*cmplx(iprealm,ipimagm)+ &
                qsurfpp*cmplx(ipprealm,ippimagm))
           iuarr(4,irow  ,icol  ,ixinterp)=  &
                -nn*(qsurfp*cmplx(iprealk,ipimagk)+ &
                qsurfpp*cmplx(ipprealk,ippimagk))
           iuarr(6,irow  ,icol  ,ixinterp)=  &
                -nn*(qsurfp*cmplx(ipreal0,ipimag0)+ &
                qsurfpp*cmplx(ippreal0,ippimag0))
           iuarr(8,irow  ,icol  ,ixinterp)=(nn*qsurfp)**2* &
                cmplx(ipprealm,ippimagm)
           iuarr(7,irow  ,icol  ,ixinterp)=(nn*qsurfp)**2* &
                cmplx(ipprealk,ippimagk)
           iuarr(9,irow  ,icol  ,ixinterp)=(nn*qsurfp)**2* &
                cmplx(ippreal0,ippimag0)

        endif
        if (compression) then
! 8/02 new terms for compression and toroidal rotation
!   these terms will need to be multiplied by various
!   factors of gamma and (m-nq) etc on the fine grid
!   when solving for v_m and vprime_m as function of
!   coefficients for u_m up_m and upp_m

! terms from LHS of v_m equation
!  first term, to be multiplied by (k-nq)(m-nq) on fine grid
           vlhs1real=g_comp*press_loc/qsurf**2*kcos(67,ip)
           vlhs1im=g_comp*press_loc/qsurf**2*ksin(67,ip)
!  2nd term, to be multiplied by Gamma^2
           vlhs2real=d_loc/f_loc*(g_comp*press_loc*kcos(68,ip)+ &
                kcos(69,ip) )
           vlhs2im=d_loc/f_loc*(g_comp*press_loc*ksin(68,ip)+ &
                ksin(69,ip) )
! terms from RHS of v_m equation
!   first term, multiplies Gamma^2 and du_m/dpsi
           vrhs1real=-d_loc/nn*ksin(10,ip)
           vrhs1im=d_loc/nn*kcos(10,ip)
!   2nd term, multiplies Gamma^2 and u_m
           vrhs2real=d_loc*(m/nn*kcos(11,ip)-2./f_loc*kcos(9,ip))
           vrhs2im=d_loc*(m/nn*ksin(11,ip)-2./f_loc*ksin(9,ip))

! terms from LHS of vprime_m equation
           vplhs1real=vlhs1real
           vplhs1im=vlhs1im
           vplhs2real=vlhs2real
           vplhs2im=vlhs2im

! terms from RHS of vprime_m eqn
           vprhs1real=nn*qsurfp*vlhs1real
           vprhs1im=nn*qsurfp*vlhs1im

! correcting apparent sign error 2/28/03
!           vprhs2real=-2.*nn*rotp_loc*vlhs2im
!           vprhs2im=2.*nn*rotp_loc*vlhs2real
!  change it back, correct due to -Gam in evp in comp_matgen
           vprhs2real=-2.*nn*rotp_loc*vlhs2im
           vprhs2im=2.*nn*rotp_loc*vlhs2real
           vprhs3real=-2.*nn*rotp_loc*vrhs1im
           vprhs3im=2.*nn*rotp_loc*vrhs1real
           vprhs4real=-2.*nn*rotp_loc*vrhs2im
           vprhs4im=2.*nn*rotp_loc*vrhs2real
           vprhs5real=vrhs1real
           vprhs5im=vrhs1im
           vprhs6real=vrhs2real
           vprhs6im=vrhs2im

! TEMP!!!!!!!!!!!!!!!!!
!           vprhs2real=0.; vprhs2im=0.
!           vprhs3real=0.; vprhs3im=0.
!           vprhs4real=0.; vprhs4im=0.


! terms multiplying vm and vmp in the um equation
           umvreal=qsurf*g_comp*press_loc* &
                (m/nn*kcos(11,ip)-2./f_loc*kcos(9,ip))
           umvim=qsurf*g_comp*press_loc* &
                (m/nn*ksin(11,ip)-2./f_loc*ksin(9,ip))
           umvpreal=-qsurf*g_comp*press_loc &
                /nn*ksin(10,ip)
           umvpim=qsurf*g_comp*press_loc &
                /nn*kcos(10,ip)
!! pbs 3/12/03 new term multiplying 1/Gamma vm in the um equation
!!  remove 3/18/03
!           umvgreal=rotp_loc*qsurf*g_comp*press_loc* &
!                kcos(10,ip)
!           umvgim=rotp_loc*qsurf*g_comp*press_loc* &
!                ksin(10,ip)


! store v and vp terms in vu array
           if (updownsym) then
              vuarr(1,irow  ,icol  ,ixinterp)=vlhs1real
              vuarr(2,irow  ,icol  ,ixinterp)=vlhs2real
              vuarr(3,irow  ,icol  ,ixinterp)=-(nn*qsurfp)*vrhs1real
              vuarr(4,irow  ,icol  ,ixinterp)=vrhs2real
              vuarr(5,irow  ,icol  ,ixinterp)=vprhs1real
              vuarr(6,irow  ,icol  ,ixinterp)=vprhs2real
              vuarr(7,irow  ,icol  ,ixinterp)=-(nn*qsurfp)*vprhs3real
              vuarr(8,irow  ,icol  ,ixinterp)=vprhs4real
              vuarr(9,irow  ,icol  ,ixinterp)=umvreal
              vuarr(10,irow ,icol  ,ixinterp)=umvpreal
! add new component for d2u/dx2 term
              vuarr(11,irow,icol,ixinterp)=(nn*qsurfp)**2*vrhs1real
! new components for du/dx terms
              vuarr(12,irow,icol,ixinterp)=-nn*qsurfp*vrhs2real
              vuarr(13,irow,icol,ixinterp)=-nn*qsurfpp*vrhs1real
!           vuarr(14,irow,icol,ixinterp)=umvgreal
           else
              vuarr(1,irow  ,icol  ,ixinterp)= &
                   cmplx(vlhs1real,vlhs1im)
              vuarr(2,irow  ,icol  ,ixinterp)= &
                   cmplx(vlhs2real,vlhs2im)
              vuarr(3,irow  ,icol  ,ixinterp)=-(nn*qsurfp)* &
                   cmplx(vrhs1real,vrhs1im)
              vuarr(4,irow  ,icol  ,ixinterp)= &
                   cmplx(vrhs2real,vrhs2im)
              vuarr(5,irow  ,icol  ,ixinterp)= &
                   cmplx(vprhs1real,vprhs1im)
              vuarr(6,irow  ,icol  ,ixinterp)= &
                   cmplx(vprhs2real,vprhs2im)
              vuarr(7,irow  ,icol  ,ixinterp)=-(nn*qsurfp)* &
                   cmplx(vprhs3real,vprhs3im)
              vuarr(8,irow  ,icol  ,ixinterp)= &
                   cmplx(vprhs4real,vprhs4im)
              vuarr(9,irow  ,icol  ,ixinterp)= &
                   cmplx(umvreal,umvim)
              vuarr(10,irow ,icol  ,ixinterp)= &
                   cmplx(umvpreal,umvpim)
! add new component for d2u/dx2 term
              vuarr(11,irow,icol,ixinterp)=(nn*qsurfp)**2* &
                   cmplx(vrhs1real,vrhs1im)
! new components for du/dx terms
              vuarr(12,irow,icol,ixinterp)=-nn*qsurfp* &
                   cmplx(vrhs2real,vrhs2im)
              vuarr(13,irow,icol,ixinterp)=-nn*qsurfpp* &
                   cmplx(vrhs1real,vrhs2im)

           endif
        endif

! 9/5/02 add additional terms due to rotation in the um equation
        if (rotation) then
! 3/12/03 pbs include factor of 2 reduction in these terms from
!   new calculation
!  3/18/03 factor of 2 was correct, return it
!  terms to be multiplied by Gamma um
           r1real=-2.*rotp_loc*qsurf*m/(nn*f_loc)*kcos(34,ip)
           r1imag=-2.*rotp_loc*qsurf*m/(nn*f_loc)*ksin(34,ip)
!  terms to be multiplied by Gamma ump
           rp1real=2.*rotp_loc*qsurf/(nn*f_loc)*ksin(33,ip)
           rp1imag=-2.*rotp_loc*qsurf/(nn*f_loc)*kcos(33,ip)

! TEMP!!!  12/15/05
!  use bug(6) to test impact of these terms
           r1real=r1real*(1.+bug(6))
           r1imag=r1imag*(1.+bug(6))
           rp1real=rp1real*(1+bug(6))
           rp1imag=rp1imag*(1.+bug(6))


! 2/14/06 pbs: add higher order rotation terms
!   proportional to omega*omega_prime

!  terms to be multiplied by um
           r2real=rotp_loc*rot_loc*( &
                -m*qsurf/nn*kcos(74,ip) &
                +qsurf*f_loc*kcos(76,ip) )
           r2imag=rotp_loc*rot_loc*( &
                -m*qsurf/nn*ksin(74,ip) &
                +qsurf*f_loc*ksin(76,ip) )
!  terms to be multiplied by ump
           rp2real=-rotp_loc*rot_loc*( &
                -qsurf/nn*ksin(73,ip) )
           rp2imag=rotp_loc*rot_loc*( &
                -qsurf/nn*kcos(73,ip) )

!!!!!!TEMP!!!!!!!!!!!!! try reversing signs
! 2/15/05 this sign reversal gives good agreement with MARS
!  and fairly good agreement with CASTOR
!   the sign reversal if probably correct but needs checking!!!!
!  9/06 sign reversal found to be correct, incorporated above
!           if (ixinterp==1 .and. m==mlo .and. mp==mlo) then
!              write(*,*) '!!!!WARNING ROT TERM SIGN REVERSAL!!!!'
!              write(outfile,*) '!!!!WARNING ROT TERM SIGN REVERSAL!!!!'
!           endif
!           r2real=-r2real
!           r2imag=-r2imag
!           rp2real=-rp2real
...
! turn off these high order terms if bug(7)=1.
           if (bug(7) .eq. 1.) then
              if (ixinterp==1 .and. m==mlo .and. mp==mlo) then
                 write(*,*) '!!!bug(7)=1, high order rot terms off!!!'
                 if (verbose .ge. 2) write(outfile,*) &
                      '!!!bug(7)=1, high order rot terms off!!!'
              endif
              r2real=0.
              r2imag=0.
              rp2real=0.
...
!!  terms to be multiplied by Gamma um
!           r1real=-rotp_loc*qsurf*m/(nn*f_loc)*kcos(34,ip)
!           r1imag=-rotp_loc*qsurf*m/(nn*f_loc)*ksin(34,ip)
!!  terms to be multiplied by Gamma ump
!           rp1real=rotp_loc*qsurf/(nn*f_loc)*ksin(33,ip)
!           rp1imag=-rotp_loc*qsurf/(nn*f_loc)*kcos(33,ip)
!! pbs 3/03 add new omegap/Gamma terms
!!   terms to be multiplied by 1/Gamma (k-nq)(m-nq) ump
!           rp2real=-rotp_loc/(nn*qsurf)*ksin(4,ip)
!           rp2imag=rotp_loc/(nn*qsurf)*kcos(4,ip)
!!   terms to be multiplied by 1/Gamma (k-nq)(m-nq) um
!           r3real=rotp_loc/(nn*qsurf)*( m*kcos(6,ip) &
!                -ksin(3,ip) )
!           r3imag=rotp_loc/(nn*qsurf)*( m*ksin(6,ip) &
!                +kcos(3,ip) )
!!   terms to be multiplied by 1/Gamma (k-nq) um
!           r4real=rotp_loc*( m*qsurfp/(nn*qsurf**2)* &
!                ksin(4,ip)-f_loc*pprime_loc*ksin(13,ip) &
!                -ffprime_loc/f_loc*ksin(12,ip) )
!           r4imag=rotp_loc*( -m*qsurfp/(nn*qsurf**2)* &
!                kcos(4,ip)+f_loc*pprime_loc*kcos(13,ip) &
!                +ffprime_loc/f_loc*kcos(12,ip) )

           if (updownsym) then
              ruarr(1,irow,icol,ixinterp)=r1real
              ruarr(2,irow,icol,ixinterp)=-nn*qsurfp*rp1real

              ruarr(3,irow,icol,ixinterp)=r2real
              ruarr(4,irow,icol,ixinterp)=-nn*qsurfp*rp2real
!           ruarr(3,irow,icol,ixinterp)=-nn*qsurfp*rp2real
!           ruarr(4,irow,icol,ixinterp)=r3real
!           ruarr(5,irow,icol,ixinterp)=r4real
           else
              ruarr(1,irow,icol,ixinterp)= &
                   cmplx(r1real,r1imag)
              ruarr(2,irow,icol,ixinterp)=-nn*qsurfp* &
                   cmplx(rp1real,rp1imag)

              ruarr(3,irow,icol,ixinterp)= &
                   cmplx(r2real,r2imag)
              ruarr(4,irow,icol,ixinterp)=-nn*qsurfp* &
                   cmplx(rp2real,rp2imag)
           endif
...
!           endif
           ...
           if(.not.updownsym) then
              asurfu(irow  ,icol  )=cmplx(asurfre,asurfim)
              asurfup(irow  ,icol  )=-nn*qsurfp* &
                   cmplx(apsurfre,apsurfim)
              isurfu(irow  ,icol  )=cmplx(isurfre,isurfim)
              isurfup(irow  ,icol  )=-nn*qsurfp* &
                   cmplx(ipsurfre,ipsurfim)

           endif
! 8/02 extra matrices needed for compressional surface terms
!   can't calculate them here because they're multipled by gamma
           if (compression) then
              ddsurf1real=press_loc*g_comp/qsurf**2*kcos(67,ip)* &
                   dmpnq*dmnq
              ddsurf1im=press_loc*g_comp/qsurf**2*ksin(67,ip)* &
                   dmpnq*dmnq
              vsurf1real=g_comp*press_loc*qsurf*ksin(10,ip)
              vsurf1im=-g_comp*press_loc*qsurf*kcos(10,ip)
              ddsurf1u(irow,icol)=ddsurf1real
              vsurf1u(irow,icol)=vsurf1real
              if(.not.updownsym) then
                 ddsurf1u(irow,icol)= &
                      cmplx(ddsurf1real,ddsurf1im)
                 vsurf1u(irow,icol)= &
                      cmplx(vsurf1real,vsurf1im)
              endif
           endif
        endif
     end do
...
!  TEMP!!!!!!!!!!
!  if (ixinterp==1) then
!     write(88,*) 'ruarr=',ruarr
!     write(88,*) 'iuarr=',iuarr
!     write(88,*) 'auarr=',auarr
!     write(88,*) 'vuarr=',vuarr
!  endif
...
subroutine oldnustuff(ixinterp)
  ...
  use elite_data, only: ns,mmax,mmin,pprime_adj,ffprime_adj,f_eq, &
       ...
       ti_eq,tiprime_eq, omegas,omegasn,omegaspi,te_eq, &
       ion_mass
  implicit none
  integer ixinterp
  integer ir,ic,ibench
...
!!$     ipimag0=0.
        ...
        if((ixinterp==1).and.(bug(2)<10.)) then
           !               if (m.eq.mmin .and. mp.eq.mmin) &
           if (m.eq.mlo .and. mp.eq.mlo .and. verbose .ge. 3) &
                write(outfile,*) 'including surface terms'
           if(mp.eq.m) then ! yes this gets defined multiple times
              b2inv_edge=kcos(13,ip)/(2.*pi) !for no good reason
              !                  write(6,*) "b2inv_edge=",b2inv_edge
           endif
