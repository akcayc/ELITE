...
!-----------------------------------------------------------------------
      use elite_data, only: nmodes,runname,lrunname,outfile,mmax,mmin, &
           nedge,dw_vac,offset_v,mindex,updownsym,verbose
      implicit none
...
!      if (updowmsym) then
!         allocate( dw_vac_loc(nmodes,nmodes) )
!      else
!         allocate( dw_vac_loc(2*nmodes,2*nmodes) )
!      endif
!      offset_v=0
      ...
      if(nmodes_v.ne.nmodes) then
         write(6,*) 'nmodes_v .ne. nmodes',nmodes_v,nmodes
         stop
      endif
      if (verbose .ge. 3) write(outfile,*) &
