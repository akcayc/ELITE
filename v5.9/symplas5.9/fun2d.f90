...
!         is now allowed
      use elite_data, only: nxinterp,ns,runname,lrunname,cdatetime, &
           ...
           vloop,qmin,zeff,gam,qa,psigrid,psixx,psimin,gamsq, &
      ...
      real, dimension(:,:),allocatable ::  f2d,rlplt,zlplt,bplplt
      ...
      if (surfplot .le. 0) then   ! don't generate *.fun2d
         if (verbose .ge. 1) write(6,*) 'no surface plot data written, surfplot=',surfplot
...
!      nxplt=2*nxinterp+1
      ...
      allocate (f2d(nxplt,ns),rlplt(nxplt,ns),zlplt(nxplt,ns),bplplt(nxplt,ns))
...
!          if (psixx(j).gt.xf) goto 10
 ...
 10     continue
      ...
      write(nunit,'(13i6)') meshtype,mmax, &
          nd,ndist,nmodes,nn,ns,nxinterp
      ...
      do  j=1,ns
         write(nunit,'(1p,6e13.5)') (f2d(i,j),i=1,nxplt)
      end do
      close(nunit)
