...
! 2/8/00 for now, f,ffprime,pprime are read from eqdat, not calculated here
      use elite_data, only: ns,outfile,rl,zl,bpl,uprimel,cosul,sinul, &
           ...
           shbishop,jedgen1store,nsurfdat,psigrid,jedgen1_arr,jedgen2_arr, &
           verbose
      implicit none
      integer nplot
...
!      if (isurf==1) then
      ...
      if (verbose .ge. 4) write(outfile,*) 'allocating surface arrays, npts= ',npts
...
!-----------------------------------------------------------------------
      ...
      qv=qv*f/(2.*pi)
      if (verbose .ge. 2) write(outfile,*) 'qv= ',qv,' q_eq=',q_eq(isurf),' qref=',qref
...
!  include adjustments due to ppmult
      if (ppmult.ne.1.) then
         if (verbose .ge. 2) write(outfile,*) 'Adjusting pprime by factor of ',ppmult
      endif
...
!     ffprime=ffprime_loc  ! for use in mercy
     if (verbose .ge. 4) write(outfile,*) 'equilibrium ffprime=',ffprime_loc
...
!     shear(isurf)=sh_loc
     ...
     if ((alphamult.ne.1.).or.(shearmult.ne.1.)) then
        ...
        if (verbose .ge. 2) write(outfile,*) &
             'adjusted alpha=',alpha_loc,' pprime=',pprime_loc
...
! can now assign adjusted values for use in mercy & nustuff
     ...
     ffprime_adj(isurf)=ffprime_loc
      ...
      if (verbose .ge. 4) write(outfile,'(1p,4(a,e12.4))') &
          ...
          '  f=',f
      if (verbose .ge. 2) &
           write(outfile,'(1p,3(a,e12.4))')'qv=',qv,' qrefp=',qrefp, &
          "  circumf=",circumf
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
      ...
      if (verbose .ge. 4) write(outfile,'(1p,3(a,e12.4))') &
...
!       if (verbose .ge. 2) write(outfile,*) 'jedge_n1=',jedge_n1
